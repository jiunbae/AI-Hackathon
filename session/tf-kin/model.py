...
class TacotronEncoder(object):
    def __init__(self,
        ...
        self.plc_embed = tf.placeholder(tf.float32, [None, max_sent_len, vector_size * 2])
    ...
    def _get_model(self):
        x1, x2 = tf.split(self.plc_embed, 2, axis=-1)

        enc1 = self._get_encoder(x1)
        enc2 = self._get_encoder(x2, reuse=True)

        stack = tf.concat((enc1, enc2), axis=-1)
        conv = tf.layers.conv1d(stack, 1, 1, 1, padding='SAME')
        conv = tf.reshape(conv, (-1, self.max_sent_len))

        return tf.layers.dense(conv, self.y_dim)
    ...
    def _get_encoder(self, in_tensor, name='enc', reuse=False):
        with tf.variable_scope(name, reuse=reuse):
            prenet = tf.layers.dense(in_tensor, self.embed_size, activation=tf.nn.relu)
